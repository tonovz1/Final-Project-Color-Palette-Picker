"""
Color Palette Picker - A tkinter GUI application for generating and exploring color palettes.
Users can generate palettes from a base color using various color harmony rules,
preview them visually, copy colors, and export them to files.
"""
...
from color_utils import (
    hex_to_rgb, rgb_to_hex, is_valid_hex, is_valid_rgb,
...
class ColorPalettePicker:
    ...
    def setup_styles(self):
        ...
        # Define custom colors for the interface
    ...
    def update_color_preview(self, event=None):
        """Update the color preview box as user types."""
    ...
    def generate_palette(self):
        """Generate palette based on base color and harmony rule."""
        hex_color = self.base_color_entry.get().strip()
    ...
    def display_palette(self):
        """Display the current palette as visual color swatches."""
        ...
        # Create a frame for swatches
        swatch_container = ttk.Frame(self.palette_frame)
        swatch_container.pack(fill=tk.BOTH, expand=True)
        
        # Display each color as a clickable swatch
        for idx, hex_color in enumerate(self.current_palette):
    ...
    def copy_to_clipboard(self, text):
        ...
        try:
            ...
            self.root.update()  # Required to persist clipboard
            self.status_label.config(text=f"Copied {text} to clipboard")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to copy to clipboard: {str(e)}")
    
    def copy_all_to_clipboard(self):
        """Copy all palette colors to clipboard."""
        if not self.current_palette:
